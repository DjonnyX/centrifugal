...
import { GradientColorPositions, NtBaseSliderPublicService, SliderStates } from 'centrifugal';

/**
 * ScrollBar component.
 * Maximum performance for extremely large lists.
 * It is based on algorithms for virtualization of screen objects.
 * @link https://github.com/DjonnyX/centrifugal/blob/main/src/app/components/custom-scrollbar/custom-scrollbar.component.ts
...
export class CustomScrollbarComponent {
    ...
    readonly api = input<NtBaseSliderPublicService>();
    ...
    constructor() {
        ...
        combineLatest([$state, $params]).pipe(
            takeUntilDestroyed(),
            tap(([state, params]) => {
                const pressed = state === SliderStates.PRESSED, hover = state === SliderStates.HOVER;
                if (pressed) {
