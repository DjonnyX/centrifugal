...
export class PageBoxComponent implements AfterViewInit, OnDestroy {
  ...
  prevData = input<IVirtualListItem<IProxyCollectionItem<IBookPageData>> | null>(null);

  nextData = input<IVirtualListItem<IProxyCollectionItem<IBookPageData>> | null>(null);
  ...
  constructor() {
    ...
    effect(() => {
      ...
      if (data && config && theme && containerElement) {
        const preset = this._themeService.getPreset(theme.bookReader.messages.message.container);
