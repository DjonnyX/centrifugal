...
import { IDisplayObjectMeasures, ISize } from 'centrifugal';
...
export class PostsGroupComponent implements OnDestroy {
  ...
  constructor() {
    ...
    effect(() => {
      ...
      if (!!theme) {
        const preset = this._themeService.getPreset(theme.newsFeed.posts.group);
        if (!!preset) {
