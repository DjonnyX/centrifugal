...
import { Id, IVirtualListCollection } from 'centrifugal';
...
interface IDB {
    ...
    groups: {
        [groupId: string]: {
            version: number;
            messages?: IVirtualListCollection<IPost>;
        }
    };
}
 ...
 * @email djonnyx@gmail.com
 */
const sortByDateTime = (a: IPost, b: IPost) => {
...
export class PostsMockService implements PostsService {
    ...
    getPosts(groupId: Id, chunk?: IPostsChunkParams): Observable<IGetPostsData> {
        ...
        let listChunk: IVirtualListCollection<IPost>;
        if (chunk) {
