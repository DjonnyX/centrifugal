...
import { PostsLoadingIndicatorComponent } from '@entities/news-feed';
import { PostsGroupComponent } from '@entities/post';
import { PostBoxComponent } from '@features/news-feed';
import { NtListComponent } from '@shared/components';
import {
  NtListModule, FocusAlignments, INtListAnimationParams, Id, IDisplayObjectConfig, IScrollEvent, IVirtualListItem,
  IVirtualListItemConfigMap, NtListPublicService, IScrollingSettings,
} from 'centrifugal';
...
import { ScrollToStartButtonComponent } from '@entities/post/scroll-to-start-button/scroll-to-start-button.component';
...
@Component({
  selector: 'x-posts',
  imports: [
    CommonModule, PostBoxComponent, PostsGroupComponent, NtListModule,
    PostsLoadingIndicatorComponent, ScrollToStartButtonComponent,
...
export class PostsComponent implements OnDestroy {
  ...
  collection = signal<Array<IProxyCollectionItem<IPostItemData>>>([]);
  protected $collection = toObservable(this.collection);

  theme: Signal<ITheme | undefined>;

  protected _proxyCollection = new ProxyCollection<IPostItemData>([]);

  animationParams: INtListAnimationParams = { scrollToItem: 25, navigateToItem: 200, navigateByKeyboard: 50, snapToItem: 150 };

  collectionConfigMap = signal<IVirtualListItemConfigMap>({});

  selectedIds = signal<Array<Id> | Id>([]);

  isLoading = signal<boolean>(true);

  isListShowed: Signal<boolean | undefined>;

  defaultItemValue = signal<IVirtualListItem<IPostItemData>>({
    dateTime: 0,
    mailed: false,
    text: '',
    edited: false,
  ...
  private _$change = new Subject<{
    item: IVirtualListItem<IProxyCollectionItem<IPostItemData>>,
  ...
  constructor() {
    ...
    effect(() => {
      ...
      if (theme && host) {
        const preset = this._themeService.getPreset(theme.newsFeed.posts);
        if (!!preset) {
    ...
    const $collection = toObservable(this.collection),
      ...
      $groupId = this._postService.$groupId,
    ...
    $virtualList.pipe(
      takeUntilDestroyed(),
      tap(list => {
        this._postService.virtualList = list;
      }),
    ).subscribe();
    ...
    $groupId.pipe(
      ...
      switchMap(groupId => {
        return of(groupId).pipe(
          ...
          switchMap(groupId => {
            return this._postsService.getPosts(groupId!, {
  ...
  onChangeItemHandler(api: NtListPublicService, { item, config, value }:
    {
      nativeEvent: any, config: IDisplayObjectConfig, item: IVirtualListItem<IProxyCollectionItem<IPostItemData>>, value: string | undefined,
    }) {
  ...
  onEditingCancelHandler(item: IVirtualListItem<IPostItemData>) {
