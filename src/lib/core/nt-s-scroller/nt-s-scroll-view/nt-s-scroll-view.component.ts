...
import {
    BehaviorSubject, combineLatest, debounceTime, delay, filter, fromEvent, map, of, race, skipWhile, startWith, Subject, switchMap,
    takeUntil, tap,
} from 'rxjs';
import {
    ...
    OVERSCROLL_EFFECT_TIME, DEFAULT_ANIMATION_PARAMS, DEFAULT_SCROLLING_ONE_BY_ONE, DEFAULT_SNAPPING_DISTANCE, DEFAULT_SNAP_TO_ITEM,
    DEFAULT_SNAP_TO_ITEM_ALIGN, EXCLUDED_CONTAINERS,
} from './const';
import { calculateDirection, matrix3d } from './utils';
import { NtSBaseScrollView } from './base';
...
/**
 * NtScrollView
 * @link https://github.com/DjonnyX/centrifugal/blob/main/src/lib/core/nt-s-scroller/nt-s-scroll-view/nt-s-scroll-view.component.ts
 * @author Evgenii Alexandrovich Grebennikov
 * @email djonnyx@gmail.com
 */
@Component({
    selector: 'nt-s-scroll-view',
    template: '',
})
export class NtSScrollView extends NtSBaseScrollView {
    ...
    readonly animationParams = input<INtSScrollViewAnimationParams>(DEFAULT_ANIMATION_PARAMS);
    ...
    constructor() {
        ...
        if (this._userInteraction) {
            ...
            $wheel.pipe(
                ...
                switchMap(() => of(this.averageVelocity)),
                debounceTime(100),
                tap(v => {
                    ...
                    this.snapIfNecessary(v, true, true, true);
                }),
            ).subscribe();
                ...
                switchMap(content => {
                    return fromEvent<WheelEvent>(content, WHEEL, { passive: false }).pipe(
                        ...
                        tap(e => {
                            ...
                            this._dragX += (this._x <= 0 || this._x >= this.scrollWidth) ? Math.abs(dragX) : 0;
                            ...
                            switchMap(e => {
                                ...
                                return fromEvent<MouseEvent>(root, MOUSE_MOVE, { passive: false }).pipe(
                                    ...
                                    switchMap(e => {
                                        ...
                                        const { position: posX, currentPos: currentPosX, endTime, scrollDelta: scrollDeltaX } =
                                            ...
                                            { position: posY, currentPos: currentPosY, scrollDelta: scrollDeltaY } =
                                        ...
                                        if (isContainerAllowedForCorrection && !!parentScroller) {
                                            parentScroller.setClientPositionOffset(!isVertical && this.scrollable ? (startClientPosX - (currentPosX ?? 0)) : 0, isVertical && this.scrollable ? (startClientPosY - (currentPosY ?? 0)) : 0);
                                        }
    ...
    private checkOverscrollByAxis(e: Event, pos: number, limit: number, force: boolean = false) {
        ...
        if (behavior === BEHAVIOR_AUTO || behavior === BEHAVIOR_SMOOTH) {
            ...
            if (isVertical) {
                if (y !== null && (this._y !== y || force)) {
                    ...
                    if (userAction) {
                        if (this.grabbing && this.scrollable) {
